...
**R4 BUILT 2026-09-05**, live on main, on a STAND-IN figure (his `YOZO vol 2.obj` was not found on any drive; he names the path, `art/sculpt/prep.py` runs on it). Built per ART.md §1, §2, §5, §6: `prep.py` (trimesh + pyfqmr decimation to a budget, box UVs in metres per vertex, normals, origin at the base centre, Draco via gltf-transform, `art/index.json` entry; stand-in 83k faces → 20k → 66 kB); `loader.model()` (GLTFLoader + Draco); library kind `sculpture` (repo GLB from the index, or a `.glb` dropped on the INVENTORY card: the world probes its size, the row shows it, typing h scales it); hold → it previews on the floor under the crosshair on its plinth, facing you, `R` turns 15° (shift back), click places, refused over another sculpture; pick up, delete, arrows (relative to where you stand), undo; HANG card rows while a sculpture is held or looked at: colour picker, texture chips (none · concrete · plaster · plywood · steel · corten · slate · checker) + tile cm, plinth on/off + w d h + colour, turn buttons; every look saved per placed copy, defaults on the library item; thumbnail rendered in-app; layout file carries pos, yaw, plinth, colour, texture and round-trips. NOT built: `upload` your own image as a texture (ART.md §2); per-group tint (ART.md open). OPEN: his OBJ path; his placement.
